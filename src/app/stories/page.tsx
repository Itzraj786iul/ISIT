        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <RevealOnView>
            <div className="mb-8 text-center sm:mb-12">
              <h1 className="mb-4 text-4xl font-bold isit-text-primary md:text-5xl">{tr('storiesHeroTitle')}</h1>
              <p className="mx-auto max-w-2xl text-lg isit-body">{tr('storiesHeroLead')}</p>
            </div>
          </RevealOnView>

          <RevealStagger className="grid grid-cols-2 gap-4 border-t border-slate-200 dark:border-cyan-300/20 pt-8 sm:grid-cols-4 sm:gap-8 sm:pt-12">
            {stats.map(({ Icon, value, label }) => (
              <div key={label} className="group text-center">
                <div className="mb-4 flex justify-center">
                  <div className="flex h-12 w-12 items-center justify-center rounded-full border border-cyan-300/25 bg-cyan-400/15 text-sky-600 dark:text-cyan-300 transition group-hover:bg-cyan-400/30">
                    <Icon size={24} aria-hidden />
                  </div>
                </div>
                <h3 className="mb-2 text-2xl font-bold isit-body md:text-3xl">{value}</h3>
                <p className="text-sm isit-body">{label}</p>
              </div>
            ))}
          </RevealStagger>
        </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <RevealOnView>
            <div className="mb-10 text-center sm:mb-16">
              <h2 className="mb-3 text-2xl font-bold isit-text-primary sm:mb-4 sm:text-3xl">{tr('storiesSectionTitle')}</h2>
              <p className="mx-auto max-w-2xl text-sm isit-body sm:text-base">{tr('storiesSectionLead')}</p>
            </div>
          </RevealOnView>
          ...
          <RevealStagger className="grid grid-cols-1 gap-6 sm:gap-8 md:grid-cols-2 lg:grid-cols-3">
            {STORIES.map((story) => (
              <article
                key={story.id}
                className="isit-card flex h-full flex-col rounded-xl p-5 motion-safe-transition duration-300 hover:-translate-y-1 hover:border-cyan-300/35 hover:shadow-xl sm:rounded-2xl sm:p-8"
              >
                <div className="mb-6 flex items-start justify-between gap-3">
                  <div>
                    <h3 className="text-lg font-bold isit-body">{story.name}</h3>
                    <div className="mt-1 flex items-center gap-1 text-xs isit-body/70">
                      <MapPin size={12} aria-hidden />
                      {story.role}
                    </div>
                  </div>
                  <Quote className="h-9 w-9 shrink-0 text-sky-600 dark:text-cyan-300/40" aria-hidden />
                </div>

                <div className="mb-5 rounded-xl border border-slate-200 dark:border-cyan-300/20 bg-slate-900/70 p-4">
                  <p className="mb-1 text-[11px] font-bold uppercase tracking-wider text-sky-600 dark:text-cyan-300">Focus</p>
                  <p className="font-semibold isit-body">{story.focus}</p>
                  <span className="mt-3 inline-block rounded-full border border-slate-200 dark:border-cyan-400/25 bg-white dark:bg-slate-950/60 px-3 py-1 text-[11px] font-semibold text-slate-600 dark:text-cyan-200/90">
                    {story.tag}
                  </span>
                </div>

                <blockquote className="mt-auto border-t border-slate-200 dark:border-cyan-300/20 pt-4 text-sm italic leading-relaxed isit-body">
        ...
        <div className="mx-auto max-w-7xl">
          <RevealOnView delayMs={50}>
            <div className="relative overflow-hidden rounded-3xl border border-cyan-300/25 bg-gradient-to-r from-cyan-600/35 to-blue-600/35 p-10 text-center text-white motion-safe-transition hover:border-cyan-300/40 md:p-16">
              ...
              <p className="relative z-10 mx-auto mb-10 max-w-2xl text-lg isit-body/90">{tr('storiesCtaLead')}</p>
